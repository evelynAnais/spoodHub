---
commonName: Regal Jumping Spider
scientificName: Phidippus regius
aliases:
  - Regal jumper
nativeRange: Southeastern United States (especially Florida), the Bahamas, Bermuda and the Greater Antilles
adultSize:
  female: 15–22 mm
  male: 12–18 mm
lifespan:
  female: 18–24 months
  male: 12–18 months
temperament: bold
difficulty: beginner
temperatureC:
  min: 21
  max: 27
humidityPct:
  min: 50
  max: 70
typicalMoltIntervalDays: 35
sources:
  - label: World Spider Catalog — Phidippus regius (C. L. Koch, 1846)
    url: https://wsc.nmbe.ch/species/31858/Phidippus_regius
    kind: database
  - label: Wikipedia — Phidippus regius
    url: https://en.wikipedia.org/wiki/Phidippus_regius
    kind: community
updated: 2026-08-18
---

The largest jumping spider in North America and by far the most commonly kept. Regals are
curious, visually driven, and unusually willing to sit and watch you back, which is most of
why they became the beginner species.

## Appearance

Adult males are black with a crisp white band across the front of the abdomen and white spots
behind it, plus iridescent green or blue chelicerae. Females are far more variable — gray,
tan, orange, or brown, sometimes with a strong apricot cast. The two are hard to tell apart
before roughly the fifth instar; after that, males develop noticeably longer, more robust
pedipalps that look like boxing gloves.

## Temperament

Bold, in the literal behavioral sense. They tend to approach rather than flee, will track
movement across a room, and often walk onto a hand offered in front of them. They are still
capable of a startled leap, so any handling should happen low over a soft surface.

Bites are rare and require real provocation — usually being squeezed. The venom is not
medically significant to humans.

## Keeping notes

- **Space matters more than most people expect.** A vertical enclosure of at least 20 × 20 × 30 cm
  suits an adult. They hunt by stalking and pouncing, and a cramped enclosure removes the
  behavior that makes them interesting to keep.
- **Ventilation over humidity.** Regals come from a warm but well-ventilated environment.
  Stagnant, damp air causes far more deaths in captivity than dryness does.
- **They need to see.** Salticids are visual hunters with excellent eyesight. A bright
  (not hot, not direct-sun) location produces a much more active spider.

## Feeding

Adults take one appropriately sized prey item every 3–4 days. Blue bottle flies are close to
ideal: nutritious, the right size, and they trigger a strong hunting response. Juveniles eat
more often — every 2–3 days — and spiderlings need *Drosophila hydei* or similar every day or
two.

Stop offering food as soon as pre-molt signs appear, and remove any uneaten live prey. A
cricket left in the enclosure with a molting spider is a genuine danger.
